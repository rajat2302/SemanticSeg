...
def testGenerator(test_path,num_image = 30,target_size = (256,256),flag_multi_class = False,as_gray = False):
    for i in range(num_image):
        img = io.imread(os.path.join(test_path,"%d.tif"%i),as_gray = False, plugin = 'pil')
        img = img / 255
...
def geneTrainNpy(image_path,mask_path,flag_multi_class = False,num_class = 2,image_prefix = "image",mask_prefix = "mask",image_as_gray = False,mask_as_gray = True):
    ...
    for index,item in enumerate(image_name_arr):
        img = io.imread(item,as_gray = image_as_gray, plugin = 'pil')
        img = np.reshape(img,img.shape + (1,)) if image_as_gray else img
        mask = io.imread(item.replace(image_path,mask_path).replace(image_prefix,mask_prefix),as_gray = mask_as_gray, plugin = 'pil')
...
def saveResult(save_path,npyfile,flag_multi_class = False,num_class = 2):
    for i,item in enumerate(npyfile):
        ...
        return os.path.join(save_path,"predict.png")